...
# Vstup:  slova s pridelenymi vzory (scripts/test_sada_podstjm.txt)
# Vystup: vsechny varianty slov ze vstupniho souboru (inputs/test_podstjm.txt)
#
...
while($line = <DESCFILE>) {
  ...
  { #ze vstupniho souboru nacitan nazev vzoru (zacina znakem : )
    ...
    open(FILEVZOR, "koncovky_sklonovani/podstjm/Šablona Vzor_".$vzor.".htm") or die "Nelze otevrit soubor ".$vzor;
    $line = "";
    $text = "";
    @koncovky = ();
    %hash = ();
